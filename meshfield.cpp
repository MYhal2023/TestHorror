...
static float		g_wave_amplitude  = 15.0f;	// 波の振幅
...
HRESULT InitMeshField(XMFLOAT3 pos, XMFLOAT3 rot,
							int nNumBlockX, int nNumBlockZ, float fBlockSizeX, float fBlockSizeZ,int fType)
{
	// ポリゴン表示位置の中心座標を設定
	g_posField = pos;
	...
	g_Center = XMFLOAT3(0.0f, 0.0f, 500.0f);		// 波の発生場所
	g_Time = 0.0f;								// 波の経過時間(＋とーとで内側外側になる)
	g_wave_frequency = 0.6f;					// 波の周波数（上下運動の速さ）
	g_wave_correction = 0.02f;					// 波の距離補正（変えなくても良いと思う）
	g_wave_amplitude = 5.0f;					// 波の振幅(波の高さ)
	int nZfield = 2;
	int nXfield = 2;
...
BOOL RayHitField(XMFLOAT3 pos, XMFLOAT3 *HitPosition, XMFLOAT3 *Normal)
{
	...
	//プレイヤー座標からメッシュフィールドのどこの部分にいるのかを割り出す
	float fz = (g_nNumBlockXField / 2.0f) * g_fBlockSizeXField;
	float fx = (g_nNumBlockZField / 2.0f) * g_fBlockSizeZField;
	int sz = (int)((-start.z + fz) / g_fBlockSizeZField);	//z方向のどこにいるのか？
	int sx = (int)(( start.x + fx) / g_fBlockSizeXField);	//x方向のどこにいるのか？
	int ez = sz + 1;
	int ex = sx + 1;
	...
	XMFLOAT3 p0 = g_Vertex[sz * (g_nNumBlockXField + 1) + sx].Position;	//四角形ポリゴンの頂点の左上
	XMFLOAT3 p1 = g_Vertex[sz * (g_nNumBlockXField + 1) + ex].Position; //四角形ポリゴンの頂点の右上
	XMFLOAT3 p2 = g_Vertex[ez * (g_nNumBlockXField + 1) + sx].Position; //四角形ポリゴンの頂点の左下
	XMFLOAT3 p3 = g_Vertex[ez * (g_nNumBlockXField + 1) + ex].Position; //四角形ポリゴンの頂点の右下
