...
static ID3D11VertexShader*		g_VertexMonoShader = NULL;		//モノトーンフィルタ頂点シェーダー
...
static ID3D11Texture2D*			depthShadowTexture = NULL;
...
ID3D11Buffer **GetView(void)
{
	return &g_ViewBuffer;
}

ID3D11Buffer **GetProjection(void)
{
	return &g_ProjectionBuffer;
}
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	//モノトーンフィルタ頂点シェーダー
	hr = D3DX11CompileFromFile("shader.hlsl", NULL, NULL, "VertexShaderMonoPolygon", "vs_4_0", shFlag, 0, NULL, &pVSBlob, &pErrorBlob, NULL);
	if (FAILED(hr))
	{
		MessageBox(NULL, (char*)pErrorBlob->GetBufferPointer(), "VS", MB_OK | MB_ICONERROR);
	}

	g_D3DDevice->CreateVertexShader(pVSBlob->GetBufferPointer(), pVSBlob->GetBufferSize(), NULL, &g_VertexMonoShader);

	// 入力レイアウト生成
	D3D11_INPUT_ELEMENT_DESC layoutMono[] =
	{
		{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT,		0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "NORMAL",   0, DXGI_FORMAT_R32G32B32_FLOAT,		0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "COLOR",    0, DXGI_FORMAT_R32G32B32A32_FLOAT,	0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
	...
	UINT numShaderElements = ARRAYSIZE(layoutMono);

	g_D3DDevice->CreateInputLayout(layoutMono,
		numShaderElements,
		pVSBlob->GetBufferPointer(),
		pVSBlob->GetBufferSize(),
		&g_VertexMonoLayout);
	...
	//水面頂点シェーダー
	hr = D3DX11CompileFromFile("shader.hlsl", NULL, NULL, "VertexShaderSpecularPolygon", "vs_4_0", shFlag, 0, NULL, &pVSBlob, &pErrorBlob, NULL);
	if (FAILED(hr))
	{
		MessageBox(NULL, (char*)pErrorBlob->GetBufferPointer(), "VS", MB_OK | MB_ICONERROR);
	}

	g_D3DDevice->CreateVertexShader(pVSBlob->GetBufferPointer(), pVSBlob->GetBufferSize(), NULL, &g_VertexSpecularShader);
	...
	UINT numWaterElements = ARRAYSIZE(layoutWater);

	g_D3DDevice->CreateInputLayout(layoutWater,
		numWaterElements,
		pVSBlob->GetBufferPointer(),
		pVSBlob->GetBufferSize(),
		&g_VertexWaterLayout);
	...
	// モノトーンフィルタピクセルシェーダコンパイル・生成
	pPSBlob = NULL;
	hr = D3DX11CompileFromFile("shader.hlsl", NULL, NULL, "PixelShaderMonoPolygon", "ps_4_0", shFlag, 0, NULL, &pPSBlob, &pErrorBlob, NULL);
	if (FAILED(hr))
	{
		MessageBox(NULL, (char*)pErrorBlob->GetBufferPointer(), "PS", MB_OK | MB_ICONERROR);
	}

	g_D3DDevice->CreatePixelShader(pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), NULL, &g_PixelMonoShader);
...
void UninitRenderer(void)
{
	...
	if (g_VertexMonoLayout)		g_VertexMonoLayout->Release();
	if (g_VertexMonoShader)		g_VertexMonoShader->Release();
...
void SwapShader(int flag)
{
	...
	case MODE_MONO:
		//モノトーンフィルタ
		g_ImmediateContext->VSSetShader(g_VertexMonoShader, NULL, 0);
